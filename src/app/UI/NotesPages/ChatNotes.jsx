...
const ChatNotes = ({ orderId }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await addNoteApi(payload).unwrap(); 
  ...
  return (
    <div className="p-4 ">
      <div className=" wave flex justify-between items-center mb-4">
        <h2 className="text-xl font-semibold ">Chat Notes </h2>
        <button
      ...
      {data?.data?.length ? (
        <div className="md:gap-5 gap-2 grid grid-cols-2 md:grid-cols-3">
          {data.data.map((note) => (
            <div key={note.id} className="p-4   bg-gray-200 rounded-xl shadow">
              <div>
