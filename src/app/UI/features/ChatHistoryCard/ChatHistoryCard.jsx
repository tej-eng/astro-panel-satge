...
const ChatHistoryCard = () => {
  ...
  const { data, error, isLoading, isFetching } = useGetExpertChatHistoryQuery({
    page,
    per_page: perPage,
    user_id: userId,
  });
  ...
  const buttonClass =
    recordList.status_name === "Completed"
  ...
  return (
    <div
      className={`${styles.callingHis} flex flex-wrap items-center justify-self-center w-[100%] flex-col gap-4`}
    >
        ...
        {uniqueData.map((card, index) => (
          ...
          >
            <div className="bg-gradient-to-r from-purple-50 via-purple-100/30 to-white p-2 md:p-4">
              <div
                className={
                  card.is_promotional === 0 || card.is_promotional === 3
                    ? "absolute top-2 right-4 flex gap-2"
                    : card.is_promotional === 1
              ...
              >
                <div className="relative group flex items-center justify-between gap-1 md:gap-2">
                  ...
                  >
                    ...
                    <div
                      className={`absolute top-5 right-[-5px] z-10 w-40 bg-white border border-gray-200 rounded-md shadow-md p-3 text-xs text-gray-700 transition-all duration-300 ${
                    ...
                    >
                      ...
                      <p>
                        <span className="font-semibold text-purple-700">DOB:</span>{" "}
                        {card.form_meta.dob || "N/A"}
                      </p>
                      <p>
                        <span className="font-semibold text-purple-700">POB:</span>{" "}
                        {card.form_meta.birth_place || "N/A"}
                      </p>
                      <p>
                        <span className="font-semibold text-purple-700">TOB:</span>{" "}
              ...
              <div className="text-sm space-y-1">
                ...
                <span className="text-gray-700">
                                     {card.form_meta?.name || "N/A"} ({card.user_id})
                </span>
                </p>
                <p>
                  ...
                  <span className="text-gray-700">
                    {" "}
                    {card.start_session_date
                      ? new Date(
                          card.start_session_date.replace(" ", "T")
              ...
              <p>
                ...
                <span className="text-gray-700">
                  ...
                  {card.request_status === "5" ? card.total_duration : "0"}
                </span>
              </p>
            ...
            {/* Review Section */}
            <div className="bg-purple-50 border border-purple-200 rounded-md p-2  text-sm text-gray-700 m-5">
              <span className="font-semibold text-purple-700">Review :</span>{" "}
              <div
                className="max-h-16 overflow-y-auto break-words text-xs p-1 md:h-[3.5rem] "
                style={{ wordBreak: "break-word", minHeight: "1.5rem" }}
              >
                {(!card.comments || card.comments === "Null")
                  ? "No review available."
                  : card.comments}
              </div>
            </div>
            ...
            <div className="flex flex-col gap-2 mt-4 sm:flex-row sm:justify-between m-5">
              {card.status_name === "Completed" && (
                <>
                  ...
                  <button
                    onClick={() =>
                      router.push(`/dashboard/chathistory/view-chat/${card.user_id}/${card.order_id}`)
                    }
                    className="flex items-center justify-center gap-1 flex-1 py-1.5 px-2 text-xs font-medium border border-blue-500 text-blue-600 rounded-md hover:bg-blue-50 transition"
                  >
              ...
              <button
                ...
                onClick={() => getKundli(card.order_id)}
              >
